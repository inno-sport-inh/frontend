...
const HistoryPage: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Собираем все уникальные имена семестров из истории и фитнес-тестов
      const semesterMap = new Map<string, StudentSemesterHistory>();
      semesterHistoryData.forEach(sem => {
        semesterMap.set(sem.semester_name, sem);
      });
      fitnessData.forEach(test => {
        if (!semesterMap.has(test.semester)) {
          // Добавляем "пустой" семестр, если его нет в истории, но есть фитнес-тест
          semesterMap.set(test.semester, {
            semester_id: Math.random(), // временный id, если нет настоящего
      ...
      // Сортируем по имени семестра (или по дате, если нужно)
      setAllSemesters(Array.from(semesterMap.values()));
    } catch (err) {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <div className="group innohassle-card p-4 bg-gradient-to-br from-floating to-primary/20 border-2 border-secondary/30 hover:border-success-500/40 transition-all duration-300 hover:shadow-lg hover:shadow-success-500/10">
          <div className="flex items-center space-x-3 mb-4">
            <div className="p-3 bg-gradient-to-br from-success-500/20 to-success-500/10 rounded-xl border border-success-500/20">
              <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-trophy text-success-500"><path d="M8 21h8"/><path d="M12 17v4"/><path d="M17 17a5 5 0 0 1-10 0"/><path d="M17 3V2a1 1 0 0 0-1-1H8a1 1 0 0 0-1 1v1"/><path d="M4 6a8 8 0 0 0 16 0"/><path d="M4 6V3a1 1 0 0 1 1-1h1"/><path d="M20 6V3a1 1 0 0 0-1-1h-1"/></svg>
            </div>
            <div>
      ...
      {/* Semesters */}
      <SemestersHistory semesterHistory={allSemesters} onSemesterClick={handleSemesterClick} />
