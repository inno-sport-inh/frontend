...
const FAQPage: React.FC = () => {
  // Keep track of which FAQ items are currently open
  const [openItems, setOpenItems] = useState<number[]>([]);
  // Controlled input for filtering questions
  ...
  // Handle opening/closing individual FAQ items
  ...
  return (
    <main className="max-w-7xl mx-auto space-y-8">
      ...
      {/* Search */}
      <section aria-label="Search FAQs" className="innohassle-card p-6">
        <div className="relative">
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-inactive" size={20} />
          <input type="text" placeholder="Search FAQ..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-primary border border-secondary rounded-xl text-contrast placeholder-inactive focus:outline-none focus:ring-2 focus:ring-brand-violet/50 focus:border-brand-violet" />
        </div>
      </section>
      ...
      <div className="space-y-6">
        {filteredCategories.map((category) => {
          ...
          return (
            <section key={category.id} aria-labelledby={`category-${category.id}-title`} className="innohassle-card overflow-hidden">
              {/* Category Header */}
              <header className="bg-primary border-b border-secondary px-6 py-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h2 id={`category-${category.id}-title`} className="text-xl font-semibold text-contrast">
                      {category.title}
                    </h2>
              ...
              {/* FAQ Items with animated accordion */}
                ...
                {category.items.map((item) => {
                  ...
                  return (
                    <li key={item.id} className="p-6">
                      <article>
                        <button onClick={() => toggleItem(item.id)} aria-expanded={isOpen} className="w-full text-left flex items-center justify-between group">
                          <div className="flex items-start space-x-3 flex-1">
                            <div className="p-1 bg-secondary rounded-full mt-1 flex-shrink-0">
                              <HelpCircle className="text-inactive" size={14} />
                            </div>
                            <h3 className="text-contrast font-medium group-hover:text-brand-violet transition-colors">{item.question}</h3>
                          </div>
                          <div className="ml-4 flex-shrink-0">{isOpen ? <ChevronUp className="text-inactive group-hover:text-brand-violet transition-colors" size={20} /> : <ChevronDown className="text-inactive group-hover:text-brand-violet transition-colors" size={20} />}</div>
                        </button>

                        {/* AnimatePresence enables smooth enter/exit animations */}
                        <AnimatePresence initial={false}>
                          {isOpen && (
                            <motion.div key="content" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="mt-4 ml-8 pr-8 overflow-hidden">
                              <div className="bg-primary/50 rounded-lg p-4 border border-secondary">
                                <p className="text-contrast leading-relaxed">{item.answer}</p>
                              </div>
                            </motion.div>
                          )}
                        </AnimatePresence>
      ...
      {/* Support Contact */}
      <aside className="innohassle-card p-6" aria-labelledby="support-heading">
        <div className="text-center">
          ...
          <h3 id="support-heading" className="text-xl font-semibold text-contrast mb-2">
            Still need help?
          </h3>
          <p className="text-inactive mb-4">Can't find the answer you're looking for? Get in touch with our support team.</p>
          <button className="innohassle-button-primary px-6 py-3">Contact Support</button>
        </div>
      </aside>

      {/* No Results */}
      {searchTerm && filteredCategories.length === 0 && (
        <section className="innohassle-card p-8 text-center">
          ...
          <p className="text-inactive">Try adjusting your search terms or browse through the categories above.</p>
        </section>
      )}
    </main>
  );
